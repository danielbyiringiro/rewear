...
if (isset($_POST['confirm_submission'])) {
    ...
    try {
        ...
        // First, insert the item into swap_items table
        $sql = "INSERT INTO swap_items (name, condition_status, brand, size, color, 
        ...
        if ($stmt->execute()) {
            ...
            // Insert a record into swap_requests table
            $sql_request = "INSERT INTO swap_requests (requested_item_id, user_id, status, created_at) 
                            VALUES (?, ?, 'pending', NOW())";

            $stmt_request = $conn->prepare($sql_request);
            $stmt_request->bind_param("ii", $new_item_id, $user_id);
            ...
            if ($stmt_request->execute()) {
                ...
                // Rollback the transaction if request insertion fails
                $conn->rollback();
                throw new Exception("Failed to create swap request");
            }
        } else {
            throw new Exception("Failed to insert swap item");
        }
    } catch (Exception $e) {
